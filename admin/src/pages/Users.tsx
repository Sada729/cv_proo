...
export default function Users() {
  ...
  async function load(p = page, q = search) {
    setLoading(true);
    try {
      const { data } = await api.get<Paginated<EndUser>>("/admin/users", { params: { page: p, search: q || undefined } });
      setUsers(data.data);
  ...
  async function remove(u: EndUser) {
  ...
  return (
    <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/60">
              {loading ? (
                <tr><td colSpan={4} className="px-5 py-10 text-center"><Loader2 className="h-5 w-5 animate-spin mx-auto text-primary" /></td></tr>
              ) : users.length === 0 ? (
      ...
      {showCreate && (
        <CreateUserModal onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); setPage(1); load(1, search); }} />
      )}
    </div>
  );
}
...
function CreateUserModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await api.post("/admin/users", { name, email, password, password_confirmation: password });
