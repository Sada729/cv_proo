...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Connexion — ResumAI" },
      { name: "description", content: "Connectez-vous à ResumAI pour créer votre CV moderne." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  async function handleGoogle() {
    ...
    if (result.error) {
      toast.error("Google : " + result.error.message);
      setLoading(false);
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signup" && (
              <div>
                ...
                <input value={fullName} onChange={e => setFullName(e.target.value)} required className="mt-1.5 w-full rounded-xl border border-input bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-primary" placeholder="Jean Diop" />
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <div className="relative mt-1.5">
                ...
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} className="w-full rounded-xl border border-input bg-background pl-10 pr-4 py-3 outline-none focus:ring-2 focus:ring-primary" placeholder="••••••••" />
              </div>
            </div>
